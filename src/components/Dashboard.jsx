...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    if (timeFilter === "week") {
      ...
      filteredTransactions = parsedTransactions.filter(
        (transaction) => new Date(transaction.id) > oneWeekAgo
      );
    } else if (timeFilter === "month") {
      const oneMonthAgo = new Date();
      oneMonthAgo.setMonth(oneMonthAgo.getMonth() - 1);
      filteredTransactions = parsedTransactions.filter(
        (transaction) => new Date(transaction.id) > oneMonthAgo
      );
    }
  ...
  const calculateStats = (transactionData) => {
    ...
    const totalRevenue = transactionData.reduce(
      (sum, transaction) => sum + parseInt(transaction.price),
      0
    );
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
              <div>
                ...
                <p className="mt-1 text-gray-600">
                  Analytics overview of your transactions
                </p>
              </div>
              {/* export to csv */}
              {(localStorage.getItem("role") === "admin" ||
                ...
                <div className="mt-4 md:mt-0">
                  <button
                    onClick={() => {
                      const csvContent =
                        ...
                        transactions
                          .map(
                            (t) =>
                              `${t.customer},${t.service},${t.outlet},${t.price}`
                          )
                          .join("\n");
                    ...
                    className="bg-primary-500 text-white px-4 py-2 rounded-md hover:bg-primary-900"
                  >
                    Export to CSV
                  </button>
                </div>
